...
## Логгирование
Для отслеживания жизненного цикла моделей и логгирования метрик в проекте в паре с s3-хранилищем использовался такоей инструмент MLOps как MLFlow.
...
- Так, в данном проекте применяется "дообучение" имеющейся модели на части новых данных и ее сравнение с текущей продуктовой версией, метрики обеих моделей логгируются в MLflow;
...
- Статистически значимая разница отклоняет нулевую гипотезу об идентичности качества сравниваемых моделей, и лучшая модель логгируется как артефакт в s3 хранилище;
- А/В тест сопровождается логгированием гистограмм распределений метрик на бутстрапе, кривых ROC_AUC и значений roc_auc_score для каждой модели;
- Так же отслеживается сам факт замены модели на "дообученную" через флаг best_model_changed flag.

Гистограммы и графики можно увидеть в папке pics проекта. 
Логгирование в MLflow выглядит следующим образом:  
