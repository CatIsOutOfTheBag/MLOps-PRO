...
Получение реальных данных о договорах и заемщиках вне банковских контуров - задача непосильная, а иногда и нелегальная :) Так, для разработки модели был выбран датасет деперсонализированных данных с сайта конкурсов по машинному обучению https://www.kaggle.com/competitions/loan-default-prediction/data

Это набор данных без указания наименования признаков, насчитывающий около 800 параметров и таргет 'loss' - указывающий одновременно и на факт дефолта, и на величину потерь в случае его реализации. Для задачи данного проекта таргет был преобразован в бинарную величину (1-был факт дефолта, 0-не было). Данные датасета соответствуют реальному набору финансовых операций, но анонимизированы, стандартизированы и лишены трендов.
...
Пайплайн решения включает в себя небольшой EDA, предобработку признаков - заполнение пропусков в соответствии с типами, построение нисходящего списка важности переменных, а также отбор наиболее информативных из них. После - оценку базового решения на кросс-валидации и forward feature selection на подмножестве отобранных признаков. Этап тюнинга гиперпараметров пропущен намеренно, в связи с тем, что классификатор хорошо себя показывает "из коробки" на дефолтных настройках.
...
## Хранение данных
В батчовых моделях финтеха заказчик всегда знает, во сколько и где должен забирать выходы из продакшн-решений. Обычно это внутренние хранилища, но в рамках проекта для получения результатов инференса, логирования артефактов и хранения наборов данных для обучения/дообучения используется s3 - объектное хранилище, разработанное компанией AWS и доступное в облачном сервисе Яндекса.
...
6. artifacts - папка хранения артефактов: гистограмм, графиков ROC_Curve, сериализованных "лучших" по результатам дообучения моделей, а также различных иных метрик и величин.
...
Чтобы в будущем добиться масштабирования проекта и освоить инструменты распределенной обработки данных, был выбран Apache Spark —  фреймворк для обработки и анализа больших объёмов информации, входящий в инфраструктуру Hadoop. Он позволяет быстро выполнять операции с данными в вычислительных кластерах и поддерживает запуск PySpark-приложений.
Основной скрипт app.py, который можно найти в папке spark проекта выполняется на Spark-кластере Data Proc Яндекс Клауда. Минимальной конфигурацией для быстрого выполнения является мастер-нода класса s3-c4-m16 и 3 compute-ноды той же конфигурации. Объемы памяти возрастают пропорционально объемам обрабатываемых датасетов (Spark хранит и обрабатывает данные в оперативной памяти).
Для корректного запуска catboost на Spark, необходимо найти соответствующий используемым в окружении версиям scala и spark пакет maven и указать его в команде spark-submit. Для данного проекта это необходимо делать следующим образом:
```sh
...
Бизнесу важно получать скор батчовых моделей в указанный интервал времени с точностью до минуты. Это связано с тем, что после скоринга на его основе запускаются внутренние сценарии, влияющие на бизнес-метрики и работу людей. Чтобы поставить процесс в проекте на расписание, использовался Apache Airflow - оркестратор, позволяющий наладить разработку, планирование и мониторинг сложных рабочих процессов. 
В основе концепции Airflow лежит DAG — направленный ациклический граф. 
В проекте даги представляют из себя следующие процессы из задач (тасок - "tasks"), выполняемых последовательно:
...
В папке dags можно найти два вышеуказанных дага, а также даг с PythonOperator и зависимостями в папке bots, где находится пример текста сообщения "Testing success", направляемого на email получателя.

Второй параметр метода ```x.login("<gmail>", "")``` пропущен (является персональными данными автора проекта) - для запуска необходимо указать свой app-password на google-account.
...
Выполнение и отладку дагов также можно наблюдать через UI:
...
## Логирование
Для отслеживания жизненного цикла моделей и логирования метрик в проекте в паре с s3-хранилищем использовался такой инструмент MLOps как MLFlow.
Жизненный цикл машинного обучения в рамках данного проекта имеет следующее обоснование:
- Известно, что модели "устаревают" или, как еще говорят "протухают" с течением времени. Зачастую это не связано напрямую с самой реализацией модели, а больше является следствием некоторого изменения в поведении данных - сдвигов в распределенениях из-за каких-либо событий или естественного flow. Тогда моделям достаточно "показать" новые данные, чтобы ее снижающееся качество предсказания вновь вернулось в прежний приемлемый интервал;
- Так, в данном проекте применяется "дообучение" имеющейся модели на части новых данных и ее сравнение с текущей продуктовой версией, метрики обеих моделей логируются в MLflow;
- Сравнение производится путем проведения А/В теста с использованием критерия Стьюдента на распределениях метрик сравниваемых моделей на бутстрапированной валидационной выборке;
- Статистически значимая разница отклоняет нулевую гипотезу об идентичности качества сравниваемых моделей, и лучшая модель логируется как артефакт в s3 хранилище;
- А/В тест сопровождается логированием гистограмм распределений метрик на бутстрапе, кривых ROC_AUC и значений roc_auc_score для каждой модели;
- Также отслеживается сам факт замены модели на "дообученную" через флаг best_model_changed flag.

Гистограммы и графики можно увидеть в папке pics проекта. 
Логирование в MLflow выглядит следующим образом:  

<image src="screens/9. mlflow ends with airflow.jpg" alt="mlflow" width="700" height="1000">

## Итоги
1. Построен и настроен полный цикл предсказания дефолта батчовой моделью, способный функционировать и покрывать требования заказчика без участия экспертов
2. Применены современные инструменты автоматизации и масштабирования, такие как Spark-кластер и PySpark, Apache Airflow, Mlflow и отправка уведомлений о выполнении процессов и поступлении новых данных
